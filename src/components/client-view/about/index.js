...
export default function ClientAboutView({ data }) {
  ...
  return (
    <div className="max-w-screen-xl mt-24 mb-6 sm:mt-14 sm:mb-14 px-6 sm:px-8 lg:px-16 mx-auto" id="about">
      ...
      <div className="flex justify-center items-center w-max gap-8">
        ...
        <AnimationWrapper className={"flex items-center w-full p-4"}>
          <motion.div
            variants={setVariants}
            className="grid gap-4 grid-cols-6 h-full max-h-[200px] w-full"
          >
            {data?.skills.split(",").map((skill) => (
              ...
              >
                <button className="whitespace-nowrap text-ellipsis overflow-hidden py-3 w-[180px] px-6 border-[2px] border-green-main bg-[#fff] text-[#000] font-semibold rounded-lg text-xl tracking-widest hover:shadow-green-main transition-all outline-none">
                  {skill}
                </button>
              </motion.div>
            ))}
